function [M] = max_pooling3d(m, m_size)
...
% size are not divisible, some image data will be lost)
    ...
    M = double(zeros(pool_r, pool_c, 3));
    ...
    for r=1:pool_r
        for c=1:pool_c
            ...
            overM = zeros(size(m));
            overM(r_ind:(r_ind+m_size - 1), c_ind:(c_ind+m_size - 1), 1:3) = double(ones(m_size, m_size, 3));
            pooled_out_m = overM .* double(m);
            % find the extracted matrix's max value and assign it to the
            % new image
            M(r, c, 1:3) = max(max(pooled_out_m));
        end
    end
end
